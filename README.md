...
- 交辦並佔住檔案：「report.md 第一段太囉唆了，幫我改短一點，這個你排進去做」。它登記時把會動到的檔佔住，另一邊動手前會先看一眼，看到有人佔著就不碰
- 換手：在 Codex 那邊說「我 Claude 額度快用完了，這邊還有什麼沒做完的嗎？有的話你接著做」，它會自己動手接著做，用的是它自己的額度
- 回頭看：「還有什麼沒做完的？」

這三句不是暗號，用自己的話講就行。安裝時 AI 會在資料夾放一份 `AIQ.md` 使用說明並讓 CLAUDE.md／AGENTS.md 指過去，之後開新對話它也認得待辦清單。結果會自己出現在下一句對話裡。最小版有登記、搶單、租約、一個檔案一個 writer、續作、結果回對話、只走訂閱；沒有暫存區寫入、整波盤點、另一引擎審查。細節在 `BUILD_SPEC.md` 第 0 節。
