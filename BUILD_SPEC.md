...
1. 建立 `AIQ.md`，內容照這個範本（可以照抄）：
   ...
   使用者不會用指令，他講的是意思，不是關鍵字。凡是他表達「把某件事排進去／排隊做／等一下做」「去跑／開始做／跑佇列」「做到哪了／進度」這類意思，就對到下面的指令，不要要求他照字面講：
   - 登記：python aiq.py add "標題" --prompt "要做什麼與驗收條件" --write <只准改的檔案或資料夾>...（他沒說範圍就先問一句只准改哪些檔）
