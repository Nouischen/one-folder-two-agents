...
1. 建立 `AIQ.md`，內容照這個範本（可以照抄）：
   ...
   ```
   # 這個資料夾有一份共用待辦清單（aiq.py），Claude Code 和 Codex 都看得到
   它存在的理由只有一個：兩邊都可能在同一個資料夾動手，必須看得見對方在做什麼、不要改到同一份檔。

   **動手改任何檔案之前，先跑一次 `<python> aiq.py status` 看一眼**（`<python>` 用安裝時確認過的解譯器完整路徑；macOS 上通常是 `python3`，Windows 上寫死那個路徑比較穩）。
   - 有 **running** 的任務會動到那些檔：另一邊正在做，停下來，告訴使用者「這幾個檔另一邊正在動，我先不碰」，問他要等還是先做別的。
   - 有 **queued** 的任務會動到那些檔：那是之前交辦、還沒開始做的事。**不要默默把舊的那件做掉當成他現在要的**，也不要另外開一件打架。先用一句話講清楚「這個檔已經有一件排著要做的事：〈標題〉，你是要我先做那件，還是換成你現在說的？」照他的回答做（要換就先 fail 或 requeue 舊的再重新登記）。
   這是這份清單最重要的用途，比登記本身更重要。

   使用者不會用指令，他講的是意思，不是關鍵字。他不會說「佇列」這種術語。凡是他表達「這個你排進去做／排一下／排進序列／等一下做／先記著」「還有什麼沒做完／接著做／繼續」「做到哪了／進度」這類意思，就對到下面的做法，不要要求他照字面講，也不要反問他是不是要用佇列。他只負責說要做什麼，該動哪些檔由你判斷：
   ...
   - **他人在現場、要你現在做（最常見）：先占住，再自己動手做。**
     順序是 python aiq.py claim --id <id> --worker <你自己是 claude 還是 codex>（把它變成 running，另一邊才看得出來有人在做），
     然後照登記的 write_scope 動手，不要碰別的檔；做完 python aiq.py done <id> --by <你自己> --result "做了什麼、怎麼驗的"。
     少了 claim 這一步，任務會一直停在「排隊中」，另一邊來接手時會以為沒人做，兩邊就同時做同一件。
...
2. 在 `CLAUDE.md` 與 `AGENTS.md` 各加下面這一段（**不要只寫「請先讀 AIQ.md」**，實測過只放指標會被跳過，AI 直接改檔）。檔案不存在就建立；已存在就附加在最後，不要覆蓋原有內容：
   ...
   Claude Code 會自動讀 CLAUDE.md，Codex 會自動讀 AGENTS.md，所以之後任何新對話都會先看到這條規則。
...
Codex：在 `~/.codex/hooks.json` 的 `hooks.UserPromptSubmit` 加同樣的指令，結構長這樣：

```json
{"hooks": {"UserPromptSubmit": [{"hooks": [{"type": "command", "command": "<python 完整路徑> <aiq.py 完整路徑> hook", "timeout": 10}]}]}}
```

**改完一定要告訴使用者：回到 Codex 裡打 `/hooks`，把這個 hook 檢視並信任一次，它才會生效**；之後只要 hook 內容有改，就要再信任一次。少了這一步，Codex 那邊會安靜地完全沒反應。
...
**跑完把測試任務收乾淨**：T1–T7 會在清單上留下測試用的任務，其中幾件宣告了 `src/` 之類的範圍。不收掉的話，使用者第一次真的交辦就會撞到「範圍衝突」，而且訊息指向一個他沒聽過的測試任務。全部 `done` 或 `fail` 掉，`status` 要是空的才回報完成。
